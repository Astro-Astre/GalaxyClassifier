...
CATALOG = "preprocessDownload/"
DATA = "preprocessDownload/"
...
if __name__ == "__main__":
    ...
    fits_hdul = GetFits(CATALOG + "galaxy_fuzzy_classifier_catalog.fits")
    ra = fits_hdul.hdul[1].data[:]['ra']
    dec = fits_hdul.hdul[1].data[:]['dec']
    gz2_fuzzy_class = fits_hdul.hdul[1].data[:]['gz2_fuzzy_class']
    dr7objid = fits_hdul.hdul[1].data[:]['dr7objid']
    redshift = fits_hdul.hdul[1].data[:]['redshift']
    if len(ra) == len(dec) == len(gz2_fuzzy_class) == len(dr7objid) == len(redshift):
        for i in range(len(ra)):
            download_from_desi(fits=True, jpg=False, g=True, r=False, z=False,
                               ...
                               label=gz2_fuzzy_class[i], dr7objid=dr7objid[i], redshift=float(redshift[i]),)
