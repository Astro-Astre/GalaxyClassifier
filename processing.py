...
def preProcessing(source_g, save_package, signal):
    ...
    for i in range(len(source)):
        ...
        #     display(save_package, signal, source_g, source_r, source_z, source[i])
        # else:
        #     # p.apply_async(display_grz,(save_package, signal, source_g, source[i],))
        #     display_grz(save_package, signal, source_g, source[i], )
        display(save_package, signal, source_g, source_r, source_z, source[i])
...
def do(label, source_dir, object_dir, signal):
    source_dir = source_dir + '/' + label + '/' + '/g'
    object_dir = object_dir + '/' + label + '/'
    preProcessing(source_dir, object_dir, signal=signal)


if __name__ == "__main__":
    source_dir = TEST_DATA
    object_dir = TEST_DATA
    do("Ec", source_dir, object_dir, signal=False)
    do("Ei", source_dir, object_dir, signal=False)
    do("Er", source_dir, object_dir, signal=False)
    do("Merger", source_dir, object_dir, signal=False)
    do("Sb", source_dir, object_dir, signal=False)
    do("SBb", source_dir, object_dir, signal=False)
    do("SBc", source_dir, object_dir, signal=False)
    do("Sc", source_dir, object_dir, signal=False)
    do("Sc_t", source_dir, object_dir, signal=False)
    do("Sd", source_dir, object_dir, signal=False)
    do("Sen", source_dir, object_dir, signal=False)
    do("Ser", source_dir, object_dir, signal=False)
    source_dir = VERIFY_DATA
    object_dir = VERIFY_DATA
    do("Ec", source_dir, object_dir, signal=False)
    do("Ei", source_dir, object_dir, signal=False)
    do("Er", source_dir, object_dir, signal=False)
    do("Merger", source_dir, object_dir, signal=False)
